...
import { Category } from '@/components/services/types'
...
import {
  ...
  Search, SlidersHorizontal, X, Camera,
  Car, Mic2, Speaker, Utensils,
} from 'lucide-react'

type Service      = AdminService
type Availability = AdminAvailability
...
export default function AdminServicesPage() {
  ...
  const displayed = filters.applyFilters(services)

  const handleAdd = (data: Omit<Service, 'id' | 'rating' | 'reviews' | 'createdAt'>) => {
    setServices((prev) => [{
      ...data, id: Date.now(), rating: 0, reviews: 0,
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-foreground/60 mt-0.5">Create and review service listings</p>
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: 'Total Services', value: total,     icon: <Package     className="w-5 h-5" />, iconBg: 'bg-blue-100    text-blue-600',    valueCls: 'text-foreground'   },
          ...
          <Card className="p-4 space-y-4">
            <div className="flex gap-3">
              ...
              <Button
                variant={filters.showFilters ? undefined : 'outline'}
              ...
              {filters.activeFilterCount > 0 && (
                <Button variant="ghost" onClick={filters.clearFilters} className="gap-1 shrink-0 text-destructive hover:text-destructive">
            ...
            {filters.showFilters && (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-3 border-t border-border">
                <div>
                  ...
                  <select title="Filter category" value={filters.category} onChange={(e) => filters.setCategory(e.target.value as Category)} className={SELECT_CLS}>
                    <option value="all">All Categories</option>
                    {CATEGORIES.map((c) => <option key={c} value={c}>{c.charAt(0).toUpperCase() + c.slice(1)}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-xs font-semibold uppercase tracking-wide text-foreground/50 mb-2 block">Price Range ($)</label>
                  <div className="flex gap-2">
                    <Input type="number" placeholder="Min" min={0} value={filters.priceMin} onChange={(e) => filters.setPriceMin(e.target.value)} />
                    <Input type="number" placeholder="Max" min={0} value={filters.priceMax} onChange={(e) => filters.setPriceMax(e.target.value)} />
                  </div>
                </div>
                <div>
                  <label className="text-xs font-semibold uppercase tracking-wide text-foreground/50 mb-2 block">Availability</label>
                  <select title="Filter availability" value={filters.availability} onChange={(e) => filters.setAvailability(e.target.value as any)} className={SELECT_CLS}>
                    <option value="all">Any</option>
                    {(['available', 'limited', 'booked'] as Availability[]).map((a) => <option key={a} value={a}>{a.charAt(0).toUpperCase() + a.slice(1)}</option>)}
                  </select>
                </div>
              </div>
            )}
          ...
          <Card className="overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3.5 border-b bg-muted/20">
              <p className="text-sm font-medium text-foreground/70">
                Showing <span className="font-bold text-foreground">{displayed.length}</span> of <span className="font-bold text-foreground">{total}</span> services
              </p>
            </div>
                    ...
                    <tr>
                      <td colSpan={8} className="px-5 py-16 text-center">
                        <div className="flex flex-col items-center gap-2 text-foreground/40">
                          ...
                          <Button variant="ghost" size="sm" onClick={filters.clearFilters} className="mt-1">Clear filters</Button>
                        </div>
                      </td>
                    </tr>
                  ) : displayed.map((s) => {
                    const cat  = CAT_STYLE[s.category] ?? CAT_STYLE.mc
                    const icon = CAT_ICON[s.category]
                    return (
                      <tr key={s.id} className="hover:bg-muted/20 transition-colors cursor-pointer" onClick={() => router.push(`/admin/services/${s.id}`)}>
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-foreground/50 mt-0.5">{s.provider}</p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-4">
                          <p className="font-bold text-primary text-base">${s.price.toLocaleString()}</p>
                        </td>
                        ...
                        <td className="px-4 py-4">
                          <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-medium capitalize ${AVAIL_STYLE[s.availability]}`}>
                            {s.availability}
                          </span>
                        </td>

                        <td className="px-4 py-4">
                          <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium capitalize ${STATUS_STYLE[s.status]}`}>
                            {s.status === 'active'    && <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block" />}
                            {s.status === 'pending'   && <span className="w-1.5 h-1.5 rounded-full bg-amber-500   inline-block" />}
                            {s.status === 'suspended' && <span className="w-1.5 h-1.5 rounded-full bg-red-500     inline-block" />}
                            {s.status}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-4">
                          {s.reviews > 0 ? (
                            <div className="flex items-center gap-1">
                              ...
                              <span className="text-foreground/40 text-xs">({s.reviews})</span>
                            </div>
                          ) : (
