...
import {
  Plus, Eye, MapPin, CalendarDays,
  BookOpen, CheckCircle, Clock, XCircle, Star,
  Search, SlidersHorizontal, X,
  Camera, Car, Mic2, Speaker, Utensils, DollarSign,
} from 'lucide-react'
...
export default function AdminBookingsPage() {
  ...
  const activeFilterCount = [category !== 'all', statusFilter !== 'all'].filter(Boolean).length
  const clearFilters = () => { setSearch(''); setCategory('all'); setStatusFilter('all') }

  const displayed = bookings.filter((b) => {
    const q = search.toLowerCase()
    if (q && !b.customerName.toLowerCase().includes(q) && !b.serviceName.toLowerCase().includes(q) && !b.providerName.toLowerCase().includes(q)) return false
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-foreground/60 mt-0.5">Monitor and manage all customer bookings</p>
            </div>
            <div className="flex items-center gap-3 shrink-0">
              <div className="text-right">
                <p className="text-xs text-foreground/50 uppercase tracking-wide font-medium">Total Revenue</p>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: 'Total Bookings', value: total,     icon: <BookOpen    className="w-5 h-5" />, iconBg: 'bg-blue-100    text-blue-600',    valueCls: 'text-foreground'   },
              { label: 'Confirmed',      value: confirmed, icon: <Star        className="w-5 h-5" />, iconBg: 'bg-blue-100    text-blue-600',    valueCls: 'text-blue-600'     },
              { label: 'Pending',        value: pending,   icon: <Clock       className="w-5 h-5" />, iconBg: 'bg-amber-100   text-amber-600',   valueCls: 'text-amber-600'    },
              { label: 'Completed',      value: completed, icon: <CheckCircle className="w-5 h-5" />, iconBg: 'bg-emerald-100 text-emerald-600', valueCls: 'text-emerald-600'  },
            ].map((stat) => (
          ...
          <Card className="overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3.5 border-b bg-muted/20">
              <p className="text-sm font-medium text-foreground/70">
                Showing <span className="font-bold text-foreground">{displayed.length}</span> of <span className="font-bold text-foreground">{total}</span> bookings
              </p>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-border">
                  {displayed.length === 0 ? (
                    ...
                    const cat  = CAT_STYLE[b.serviceCategory] ?? CAT_STYLE.mc
                    const icon = CAT_ICON[b.serviceCategory]
                    return (
                      <tr key={b.id} className="hover:bg-muted/20 transition-colors cursor-pointer" onClick={() => router.push(`/admin/bookings/${b.id}`)}>

                        {/* Customer */}
                        <td className="px-5 py-4">
                          <p className="font-semibold text-foreground leading-tight">{b.customerName}</p>
                          <p className="text-xs text-foreground/50 mt-0.5">{b.customerEmail}</p>
                        </td>

                        {/* Service */}
                        <td className="px-4 py-4">
                          <div className="flex items-center gap-2">
                            <div className={`w-7 h-7 rounded-md flex items-center justify-center shrink-0 ${cat.icon}`}>
                              {icon}
                            </div>
                            <div>
                              <p className="font-medium leading-tight line-clamp-1">{b.serviceName}</p>
                              <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium mt-0.5 ${cat.pill}`}>
                                <span className="capitalize">{b.serviceCategory}</span>
                              </span>
                            </div>
                          </div>
                        </td>
                        ...
                        {/* Provider */}
                        <td className="px-4 py-4">
                          <p className="text-foreground/70">{b.providerName}</p>
                          <span className="flex items-center gap-1 text-xs text-foreground/40 mt-0.5">
                        ...
                        <td className="px-4 py-4">
                          <span className="flex items-center gap-1.5 text-foreground/70">
                            <CalendarDays className="w-3.5 h-3.5 text-foreground/40 shrink-0" />
                            {b.eventDate}
                          </span>
                          <p className="text-xs text-foreground/40 mt-0.5">Booked {b.createdAt}</p>
                        </td>
                        ...
                        {/* Status */}
                        <td className="px-4 py-4">
                          <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium capitalize ${STATUS_STYLE[b.status]}`}>
                            {b.status === 'pending'   && <span className="w-1.5 h-1.5 rounded-full bg-amber-500   inline-block" />}
                            {b.status === 'confirmed' && <span className="w-1.5 h-1.5 rounded-full bg-blue-500    inline-block" />}
                            {b.status === 'completed' && <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block" />}
                            {b.status === 'cancelled' && <span className="w-1.5 h-1.5 rounded-full bg-red-500     inline-block" />}
                            {b.status}
                          </span>
                        </td>
